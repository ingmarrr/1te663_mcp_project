...
*****************************************************************************/
#define LCD_CS_PIN   PB1  // Define LCD Chip Select pin
#define RFID_CS_PIN  PB2  // Define RFID Chip Select pin

// Macros to control CS lines
#define LCD_CS_LOW()  (PORTB &= ~(1 << LCD_CS_PIN))   // Activate LCD
#define LCD_CS_HIGH() (PORTB |= (1 << LCD_CS_PIN))    // Deactivate LCD
#define RFID_CS_LOW()  (PORTB &= ~(1 << RFID_CS_PIN)) // Activate RFID
#define RFID_CS_HIGH() (PORTB |= (1 << RFID_CS_PIN))  // Deactivate RFID)
#include <inttypes.h>
#include <avr/io.h>
...
void lcd_command(uint8_t cmd) {
    RFID_CS_HIGH();  // Ensure RFID is not active
    LCD_CS_LOW();    // Activate LCD
    lcd_waitbusy();
    lcd_write(cmd, 0);
